...
static unsigned int		get_burningship_point_color(
							...
							int y)
{
	...
	k[0] = ((double)x - MWW / 2) / pgv->view.zoom * 0.01
		+ pgv->view.position.x * 10;
	k[1] = ((double)y - MWH / 2) / pgv->view.zoom * 0.01
		+ pgv->view.position.y * 10;
	d[2] = (pgv->view.mouse_position.x - MWW / 2.0) / MWW / 5;
	d[3] = (pgv->view.mouse_position.y - MWH / 2.0) / MWH / 5;
	i = -1;
	while (++i < pgv->max_iter)
	{
		d[0] = d[2] * d[2] - d[3] * d[3] + k[0];
		d[1] = ft_abs_d(2 * d[2] * d[3]) + k[1];
		d[2] = d[0];
		d[3] = d[1];
	...
	while ((y += pdtp->pgv->view.show_steps) < ymax)
	{
		...
		while (++x[1] < xmax)
		{
			((unsigned int *)pdtp->pgv->winbuf.img_data.ptr)
				...
				get_burningship_point_color(pdtp->pgv, d, x[1], y);
		}
	}
}
