...
static unsigned int		get_j3and2_point_color(
							...
							int y)
{
	...
	k[0] = 0.0 + (pgv->view.mouse_position.y - MWH / 2.0) / MWH * 4.0;
	k[1] = 0.0 + (pgv->view.mouse_position.x - MWW / 2.0) / MWW * 4.0;
	d[2] = (x - MWW / 2.0) / pgv->view.zoom * 0.01 + pgv->view.position.x * 10;
	d[3] = (y - MWH / 2.0) / pgv->view.zoom * 0.01 + pgv->view.position.y * 10;
	i = -1;
	while (++i < pgv->max_iter)
	{
		d[0] = (d[2] + 1) * d[2] * d[2] - (3.0 * d[2] + 1) * d[3] * d[3] + k[0];
		d[1] = (3.0 * d[2] + 2) * d[2] * d[3] - d[3] * d[3] * d[3] + k[1];
		d[2] = d[0];
		d[3] = d[1];
	...
	while ((y += pdtp->pgv->view.show_steps) < ymax)
	{
		...
		while (++x[1] < xmax)
		{
			((unsigned int *)pdtp->pgv->winbuf.img_data.ptr)
				...
				get_j3and2_point_color(pdtp->pgv, d, x[1], y);
		}
	}
}
